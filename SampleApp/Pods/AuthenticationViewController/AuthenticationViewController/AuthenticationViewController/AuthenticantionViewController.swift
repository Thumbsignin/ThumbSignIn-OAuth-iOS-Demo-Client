...
open class AuthenticationViewController: SFSafariViewController {
	...
	*/
    ...
    //let request = NSMutableURLRequest(url: provider.accessTokenURL as URL, cachePolicy:  NSURLRequest.CachePolicy.reloadIgnoringLocalAndRemoteCacheData, timeoutInterval: 5.0)
		let parameters = provider.parameters.map { key, value in
			"\(key)=\(value)"
			}.joined(separator: "&")
		let data = "client_id=\(provider.clientId)&client_secret=\(provider.clientSecret)&code=\(code)&\(parameters)&response_type=token"
		
    let headerData = "\(provider.clientId):\(provider.clientSecret)"
		request.httpMethod = "POST"
		request.httpBody = data.data(using: String.Encoding.utf8)
    let d = headerData.data(using: String.Encoding.utf8)
    request.setValue("Basic \(d!.base64EncodedString())", forHTTPHeaderField: "Authorization")
